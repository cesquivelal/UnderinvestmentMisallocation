...
function SubsidyCost(z::Float64,kN::Float64,kT::Float64,b::Float64,
                       SOL_PLA::Solution,GRIDS::Grids,par::Pars)
    @unpack itp_kNprime, itp_kTprime, itp_bprime, itp_q1 = SOL_PLA
    #Optimal subsidy rate
    TAU_N, TAU_T=Optimal_Subsidies(z,kN,kT,b,SOL_PLA,GRIDS,par)
    #Optimal subsidy cost
    kNprime=itp_kNprime(b,kT,kN,z)
    kTprime=itp_kTprime(b,kT,kN,z)
    S_N=TAU_N*(kNprime-(1-par.δ)*kN)
    S_T=TAU_T*(kTprime-(1-par.δ)*kT)
    return S_N, S_T
end
...
function SubsidyCostSeries(S0::States_TS,SOL_PLA::Solution,GRIDS::Grids,par::Pars)
    T=length(S0.z)
    S_N=Array{Float64,1}(undef,T)
    S_T=Array{Float64,1}(undef,T)
    for t in 1:T
        if S0.Def[t]==1
            S_N[t]=0.0
            S_T[t]=0.0
            ...
            kT=S0.KT[t]
            b=S0.B[t]
            S_N[t], S_T[t]=SubsidyCost(z,kN,kT,b,SOL_PLA,GRIDS,par)
        end
    end
    return S_N, S_T
end
...
@with_kw mutable struct Moments_Tau
    ...
    AvTau_N::Float64 = 0.0
    AvTau_T::Float64 = 0.0
    #Average subsidy/tax cost/revenue
    AvS_N_gdp::Float64 = 0.0
    AvS_T_gdp::Float64 = 0.0
    #Cyclicality of net subsidy expenditure
    Corr_GDP_STotal::Float64 = 0.0
    Corr_Spreads_STotal::Float64 = 0.0
end

function OptimalTaxMomentsOnce(SOL_PLA::Solution,SOL_DEC::Solution,GRIDS::Grids,par::Pars)
    #Get path of states with same criteria as for moments
    S0=GetOnePathForMoments(SOL_DEC,GRIDS,par)
    #Get the corresponding path of taxes
    TAU_N, TAU_T=OptimalSubsidiesSeries(S0,SOL_PLA,GRIDS,par)
    S_N, S_T=SubsidyCostSeries(S0,SOL_PLA,GRIDS,par)
    ...
    #Average subsidy/tax rate
    AvTau_N=100*mean(TAU_N)
    AvTau_T=100*mean(TAU_T)
    #Average subsidy/tax cost/revenue
    AvS_N_gdp=100*mean((P_TS .* S_N) ./ mean(GDP_TS))
    AvS_T_gdp=100*mean((P_TS .* S_T) ./ mean(GDP_TS))
    #Cyclicality of subsidy/tax expenditure/revenue
    log_GDP=log.(abs.(GDP_TS))
    GDP_trend=hp_filter(log_GDP,HPFilter_Par)
    GDP_cyc=100.0*(log_GDP .- GDP_trend)
    STOTAL=100*(P_TS .* (S_T + S_N)) ./ mean(GDP_TS)
    Corr_GDP_STotal=cor(GDP_cyc,STOTAL)
    Corr_Spreads_STotal=cor(S0.Spreads,STOTAL)
    return Moments_Tau(AvTau_N,AvTau_T,AvS_N_gdp,AvS_T_gdp,Corr_GDP_STotal,Corr_Spreads_STotal)
end

function AverageTauMomentsManySamples(SOL_PLA::Solution,SOL_DEC::Solution,GRIDS::Grids,par::Pars)
    @unpack NSamplesMoments = par
    Random.seed!(1234)
    #Initiate them at 0.0 to facilitate average across samples
    MOMENTS=Moments_Tau()
    for i in 1:NSamplesMoments
        # println("Sample $i for moments")
        MOMS=OptimalTaxMomentsOnce(SOL_PLA,SOL_DEC,GRIDS,par)
        #Default, spreads, and Debt
        MOMENTS.AvTau_N=MOMENTS.AvTau_N+MOMS.AvTau_N/NSamplesMoments
        MOMENTS.AvTau_T=MOMENTS.AvTau_T+MOMS.AvTau_T/NSamplesMoments
        MOMENTS.AvS_N_gdp=MOMENTS.AvS_N_gdp+MOMS.AvS_N_gdp/NSamplesMoments
        MOMENTS.AvS_T_gdp=MOMENTS.AvS_T_gdp+MOMS.AvS_T_gdp/NSamplesMoments
        MOMENTS.Corr_GDP_STotal=MOMENTS.Corr_GDP_STotal+MOMS.Corr_GDP_STotal/NSamplesMoments
        MOMENTS.Corr_Spreads_STotal=MOMENTS.Corr_Spreads_STotal+MOMS.Corr_Spreads_STotal/NSamplesMoments
    end
    return MOMENTS
end
